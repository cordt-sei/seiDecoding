...
function generateSelectorMap(abis: Record<string, any>) {
  const selectorMap: Record<string, { abi: any, functionName: string }> = {};
...
async function fetchAndDecodeTx(url: string) {
  try {
...
function isEvmTransaction(type: string): boolean {
  return type.startsWith('/seiprotocol.seichain.evm') || type.startsWith('/seiprotocol.seichain.eth');
}

async function decodeEvmTransaction(data: string) {
...
async function decodeCosmosTransaction(message: any) {
  try {
    const messageType = message['@type'];
